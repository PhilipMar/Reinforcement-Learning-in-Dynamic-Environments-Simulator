...
/**
 * Parses the {@link Criterion} string for level change criteria in the config file.
 */
public class LevelChangeCriteriaHandler implements KeyHandler {

    public static final String packagePath = MaxEpisodesReached.class.getPackage().getName() + ".";
    ...
    public boolean handle(String key, String value, Config config) {
        ...
        if (identifiers.getLevelChangeCriteria().equals(key)) {
            for (String parameterAsString : HandlerUtils.splitCriteriaString(value, key)) {
                ...
                try {
                    config.levelChangeCriteria.add((Criterion) HandlerUtils.createClass(packagePath, className, parsedArgumentMap, interfaceName));

                } catch (ClassCastException e) {
                    throw new HandlerUtils.ConfigurationReaderException("The class [" + className + "] does not implement the interface [" + interfaceName + "]." +
