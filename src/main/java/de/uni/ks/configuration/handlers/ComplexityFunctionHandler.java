...
/**
 * Parses the {@link ComplexityFunction} string in the config file.
 */
public class ComplexityFunctionHandler implements KeyHandler {

    public static final String packagePath = ComplexityFunction.class.getPackage().getName() + ".";
    public static final String interfaceName = ComplexityFunction.class.getName();
    ...
    public boolean handle(String key, String value, Config config) {
        ...
        if (identifiers.getComplexityFunction().equals(key)) {

            String className = HandlerUtils.parseClassName(value);
            Map<String, String> parsedArgumentMap = HandlerUtils.parseParameters(value);

            try {
                config.complexityFunction =
                        (ComplexityFunction) HandlerUtils.createClass(packagePath, className,
                                parsedArgumentMap, interfaceName);
